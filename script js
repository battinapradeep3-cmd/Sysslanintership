...
function addConfirmationQR(booking) {
    ...
    const details =
        document.querySelector("#confirmationModal .confirmation-details");
    ...
    let qrBox =
        document.getElementById("aureliaConfirmationQR");
    ...
    if (!qrBox) {
        ...
        details.parentNode.insertBefore(qrBox, details.nextSibling);
    }

    qrBox.innerHTML = `
        <div style="font-size:9px;letter-spacing:1.5px;color:#aaa39a;margin-bottom:10px;">SCAN TO VERIFY TICKET</div>
        <img src="${createQRCodeData(booking)}" alt="QR code for ${escapeHTML(booking.id)}" style="width:180px;height:180px;margin:auto;background:#fff;padding:6px;">
        <div style="font-size:10px;color:#c9a96e;margin-top:8px;">${escapeHTML(booking.id)}</div>
    `;
}
    ...
    if (bookings.length === 0) {
        ...
        container.innerHTML = `
            <div class="empty-tickets">
                ...
                <p>Sign in and book an event to see your digital tickets here.</p>
    ...
    bookings.forEach(function(booking) {
        ...
        card.innerHTML = `
            <div class="ticket-info">
                ...
                <p>${escapeHTML(booking.date)} · ${escapeHTML(booking.time)}</p>
                <p>${escapeHTML(booking.location)}</p>
                <p>${escapeHTML(booking.venue)}</p>
                <p>${booking.quantity} × ${escapeHTML(formatTicketType(booking.ticketType))}</p>
                <strong>₹${booking.total}</strong>
                <p class="ticket-id">Booking ID: ${escapeHTML(booking.id)}</p>

                <div style="margin-top:18px;text-align:center;padding:12px;border-top:1px solid rgba(255,255,255,.10);">
                    <img src="${createQRCodeData(booking)}"
                         alt="QR code for ${escapeHTML(booking.id)}"
                         style="width:140px;height:140px;background:#fff;padding:5px;margin:auto;">
                    <small style="display:block;margin-top:7px;">SCAN TO VERIFY</small>
                </div>
    ...
    container
        ...
        .forEach(function(button) {
            button.addEventListener("click", function() {
                cancelTicket(button.getAttribute("data-cancel-ticket"));
            });
        });
}
